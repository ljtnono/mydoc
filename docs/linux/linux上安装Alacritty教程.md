...
> 本教程参考链接：[https://github.com/alacritty/alacritty/blob/master/INSTALL.md](https://github.com/alacritty/alacritty/blob/master/INSTALL.md) 

## 测试环境

![](http://f.lingjiatong.cn:30090/rootelement/articleQuote/16758447009023.jpg)
...
# 在 Linux/BSD 上，如果需要在不支持 X11 或 Wayland 渲染后端的情况下构建 Alacritty，可以使用以下命令。
...
以下操作需要在alacritty的仓库目录中进行

1、为确保 Alacritty 正常工作，必须使用 alacritty 或 alacritty-direct terminfo。如果安装了 alacritty terminfo，它将被自动拾取。 如果以下命令返回且没有任何错误，则说明已经安装了 alacritty terminfo：
...
许多 Linux 和 BSD 发行版都支持用于将应用程序添加到系统菜单的桌面条目。这将为 Alacritty 安装桌面条目：
...
仓库地址：[https://github.com/eendroroy/alacritty-theme](https://github.com/eendroroy/alacritty-theme)
...
2、在~/.alacritty.yml配置文件中配置相应的主题

```yml

# 其中scheme_name为具体的文件名
import:
